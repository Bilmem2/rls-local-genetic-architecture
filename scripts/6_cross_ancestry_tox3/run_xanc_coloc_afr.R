#!/usr/bin/env Rscript
# Cross-ancestry (AFR) local genetic correlation at the EUR coloc loci, using the AFR data
# we already have. Targeted (locus-specific) like the TOX3 replication; RLS-AFR underpowered
# (n_cases 2,176) so this is a directional check, not definitive.
...
info_row <- function(ph) sprintf("%s NA NA %s", ph, file.path(M, paste0(ph, ".sumstats.gz")))
res <- list()
for (J in JOBS) {
  a <- J$a; t <- J$t

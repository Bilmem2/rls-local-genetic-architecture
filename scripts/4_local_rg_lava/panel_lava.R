...
#   Phenotype 1 of each pair: rls_akcimen (primary RLS, biggest) or pd_nalls (primary PD).
...
# parse an LDSC --rg .log: summary table (per-trait h2_int + gcov_int) and phen1's own intercept
...
build_ovl <- function(file,phen1,trait,tab,aint){ r<-tab[tab$trait==trait,]
  ...
  m<-matrix(c(aint,g,g,ht),2,dimnames=list(c(phen1,trait),c(phen1,trait)))
...
rls_tab<-read_rg_tab(rls_log); rls_aint<-phen1_intercept(rls_log)
pd_tab <-read_rg_tab(pd_log);  pd_aint <-phen1_intercept(pd_log)
...
# priority-ordered pairs: tag, phen1, trait, src(rls|pd)
PAIRS <- list(
...
for(P in PAIRS){
  tag<-P[1]; phen1<-P[2]; trait<-P[3]; src<-P[4]
  ...
  if(!tryCatch({build_ovl(ovl,phen1,trait,tab,aint);TRUE},error=function(e){cat(" ovl err",tag,conditionMessage(e),"\n");FALSE})) next
  cat(sprintf("\n>>> [%s] %s x %s\n",tag,phen1,trait)); flush.console()
  inp <- tryCatch(process.input(info,ovl,REF,c(phen1,trait)),error=function(e){cat(" input err:",conditionMessage(e),"\n");NULL})
  if(is.null(inp)) next
  res<-list(); np<-0; ng<-0
  for(i in 1:n){
    ...
    if(any(u$phen==phen1 & u$p<ut) && any(u$phen==trait & u$p<ut)){
      ng<-ng+1

...
# the two case-control disorders carry case/control counts; narcolepsy treated continuous (local rho scale-invariant)
...
# tag, phen1, trait, rg-log (narco-specific logs produced by the munge step)
PAIRS <- list(
...
first <- TRUE
for(P in PAIRS){
  tag<-P[1]; phen1<-P[2]; trait<-P[3]; rglog<-P[4]
  tab<-read_rg_tab(rglog); aint<-tryCatch(phen1_intercept(rglog),error=function(e) NA)
  ...
  cat(sprintf("\n>>> [%s] %s x %s (phen1 intercept=%.4f)\n",tag,phen1,trait,aint)); flush.console()
